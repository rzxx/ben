void main() {
  float historyWeight = clamp(uniforms.params.x, 0.0, 0.98);
  float response = max(uniforms.params.y, 0.001);
  float clampRange = max(uniforms.params.z, 0.001);
  float enabled = uniforms.params.w;
