...
const compositePhaseCycleLength = 4;
const compositePhaseHoldFrames = 4;
const compositeShiftedDitherBlend = 0.2;
...
export function createBackgroundShaderRenderer(
  ...
  const stopRenderLoop = () => {
    ...
    renderState.compositePhase = 0;
    renderState.compositePhaseFrameCount = 0;
  };
  ...
  const writeCompositeUniforms = (
    ...
    uniformData[4] = Math.max(0, ignStrength);
    uniformData[5] = phase;
    uniformData[6] = compositeShiftedDitherBlend;
    uniformData[7] = 0;
  ...
  const render = (time: number) => {
    ...
    writeCompositeUniforms(
      settingsValue.opacity,
      settingsValue.ignStrength,
      renderState.compositePhase,
    );
    ...
    gl.bindVertexArray(vao);
    gl.drawArrays(gl.TRIANGLES, 0, 3);
    renderState.compositePhaseFrameCount += 1;
    if (renderState.compositePhaseFrameCount >= compositePhaseHoldFrames) {
      renderState.compositePhaseFrameCount = 0;
      renderState.compositePhase =
        (renderState.compositePhase + 1) % compositePhaseCycleLength;
    }
