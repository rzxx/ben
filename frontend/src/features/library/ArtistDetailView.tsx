  ...
  return (
    <section className="">
      <button
        ...
        className="text-theme-600 hover:text-theme-800 dark:text-theme-400 dark:hover:text-theme-200 inline-flex w-fit items-center gap-2 rounded-md px-1 py-1 text-sm transition-colors"
      >
        <ArrowLeft size={14} />
        Back to artists
      </button>

      <div className="bg-theme-100/80 dark:border-theme-800 dark:bg-theme-950/15 rounded-xl border border-black/7 p-4">
        <p className="text-theme-600 dark:text-theme-400 text-xs tracking-wide uppercase">
          Artist
        </p>
        <h1 className="text-theme-900 dark:text-theme-100 text-2xl font-semibold">
          {artist.name}
        </h1>
        <p className="text-theme-600 dark:text-theme-400 text-sm">
        ...
        <button
          ...
          className="bg-theme-900 text-theme-100 hover:bg-theme-800 dark:bg-theme-100 dark:text-theme-900 dark:hover:bg-theme-200 mt-3 inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium"
        >
          <Play size={16} />
          Play Artist Songs
        </button>
      ...
      <section className="bg-theme-100/80 dark:border-theme-800 dark:bg-theme-950/15 rounded-xl border border-black/7 p-4">
        <h2 className="text-theme-900 dark:text-theme-100 mb-3 text-sm font-semibold">
          Most Liked Tracks
        </h2>
        {props.topTracks.length === 0 ? (
          ...
          <ul className="flex flex-col gap-2">
            {props.topTracks.slice(0, 5).map((track, index) => (
              <li
                key={track.trackId}
                className="bg-theme-100 dark:border-theme-800 dark:bg-theme-950/15 flex items-center gap-3 rounded-md border border-black/7 px-3 py-2"
              >
                <p className="text-theme-600 dark:text-theme-500 w-6 text-xs">
                  {index + 1}
                </p>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {artist.albums.map((album) => (
            <button
              ...
              className="bg-theme-100/80 hover:border-theme-500 dark:border-theme-800 dark:bg-theme-950/15 dark:hover:border-theme-600 rounded-lg border border-black/7 p-3 text-left transition"
            >
              <CoverArt
